...
ITEM_ID=$(gh project item-list $PROJECT_NUMBER --owner $OWNER --format json --limit 100 | \
  ...
  target = '$ISSUE_URL_OR_NUM'; \
  # Match either Full URL or Issue Number (as string)
  items = [i for i in data['items'] if i['content']['url'] == target or str(i['content']['number']) == target]; \
